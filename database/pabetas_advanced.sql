...
ALTER TABLE questions ADD COLUMN IF NOT EXISTS group_label VARCHAR(80) NOT NULL DEFAULT 'Umum' AFTER type;
ALTER TABLE questions ADD COLUMN IF NOT EXISTS difficulty ENUM('mudah','sedang','sulit') NOT NULL DEFAULT 'mudah' AFTER group_label;
...
CREATE TABLE IF NOT EXISTS learning_flow_steps (
  ...
  title VARCHAR(150) NOT NULL,
  description TEXT NULL,
  action_type ENUM('pretest','material','live_game','posttest','remedial','quiz','custom') NOT NULL DEFAULT 'custom',
...
SELECT * FROM (
  SELECT 1,'Pretest','Tes awal untuk mengukur pemahaman sebelum pembelajaran.','pretest','',1,1 UNION ALL
  SELECT 2,'Pembelajaran Papan Belajar','Siswa membaca materi satuan panjang, melihat tabel ke bawah, dan mencoba contoh soal.','material','',1,1 UNION ALL
  SELECT 3,'Live Game','Siswa masuk game seperti Quizizz memakai kode dari guru.','live_game','',1,1 UNION ALL
  SELECT 4,'Posttest','Tes akhir untuk melihat peningkatan nilai setelah belajar.','posttest','',1,1 UNION ALL
  SELECT 5,'Remedial Otomatis','Latihan penguatan bagi siswa yang belum tuntas.','remedial','',0,1 UNION ALL
...
ALTER TABLE learning_flow_steps MODIFY action_type ENUM('pretest','material','live_game','posttest','remedial','quiz','reflection','custom') NOT NULL DEFAULT 'custom';
INSERT INTO learning_flow_steps(step_order,title,description,action_type,action_payload,is_required,status)
